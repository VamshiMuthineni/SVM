...
% Last modified: 30-Sep-2018
    ...
    methods (Static)
        ...
        function [D, lb, imRegs] = getNegDataPoints(k)
            rng(1234); % reset random generator. Keep same seed for repeatability
            load(sprintf('%s/%sAnno.mat', HW4_Utils.dataDir, 'train'), 'ubAnno');
            [negD, negRegs] = deal(cell(1, length(ubAnno)));            
            ...
            for i=1:length(ubAnno)
                ...
                ubs = ubAnno{i}; % annotated upper body                
                % sample k random patches; some will be used as negative exampels
                % Choose k sufficiently large to ensure successk = 1000;
                [imH, imW,~] = size(im);
                randLeft = randi(imW, [1, k]);
                randTop = randi(imH, [1, k]);
                randSz = randi(min(imH, imW), [1, k]);
...
%                nNeg2SamplePerIm = floor(k/length(ubAnno));
                %Now extract features for some few random patches
                [~,numrec] = size(randRects);
                if (numrec>20)
                    nNeg2SamplePerIm = 20;
                else
                    nNeg2SamplePerIm = numrec;
                end
                g = floor((numrec-20)*rand(1))+1;
                [D_i, R_i] = deal(cell(1, nNeg2SamplePerIm));
                %numrec-nNeg2SamplePerIm+1:numrec;
                for j=g:g+nNeg2SamplePerIm
                    imReg = im(randRects(2,j):randRects(4,j), randRects(1,j):randRects(3,j),:);
                    imReg = imresize(imReg, HW4_Utils.normImSz);
                    R_i{j} = imReg;
                    D_i{j} = HW4_Utils.cmpFeat(rgb2gray(imReg));                    
            ...
            negD = cat(2, negD{:});   
            D = HW4_Utils.l2Norm(double(negD));
            lb = -ones(size(negD,2), 1);
            imRegs = negRegs{:};
        end;
